// Both generators over one charset, one font folder and one out folder.
//
...
//   npm run gen:fonts -- --fonts ./fonts       read that folder instead of textgen/fonts/
//   npm run gen:fonts -- --out ./atlases       write msdf/ and polygons/ under that folder
//
// A script rather than `gen:msdf && gen:polygons`: npm appends a `--` argument to the LAST
// command of a chain, so the second generator would get the arguments and the first would run on
// the defaults. The two atlases covering different characters is the one thing that must not
// happen - a node switching between the MSDF and the vector path would change which glyphs are
// missing - so one process reads the command line and hands both generators the same answers.
