...
// ray). Run with `npx vitest run packages/engine/src/math/math.test.ts` to verify their behavior.
...
it('Quaternion (+90 deg about Y takes +X to -Z in a right-handed frame)', () => {
  {
...
it('Matrix4x4 inverse round-trips a point through a TRS matrix.', () => {
...
it('Transform toMatrix agrees with TRS; identity forward is -Z.', () => {
  {
...
it('Transform compose parent*child applied to a point equals applying', () => {
...
it('AABB containment, overlap, and transform-refit.', () => {
...
it('Ray vs AABB slab test: front hit, miss, inside-origin, flat box.', () => {
  {
...
it('one 2D vector: the class and its data shape are the same two fields', () => {
