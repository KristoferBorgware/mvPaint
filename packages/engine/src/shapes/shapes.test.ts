...
//   npx vitest run packages/engine/src/shapes/shapes.test.ts
...
// it rotates about.
it('where each shape\'s origin sits', () => {
...
it('boxFromPoints: fits an oriented box in a turned frame', () => {
...
it('resizeFactors: the opposite anchor holds still, and dragging to the start is a no-op', () => {
    const box: OrientedBox = { cx: 0, cy: 0, halfW: 10, halfH: 5, rotation: 0 }
...
it('resize on a ROTATED box works in the box\'s own frame', () => {
...
it('applyWorldTransform: a world delta lands correctly on the node\'s own fields', () => {
    // Pure translation.
...
it('the delta reaches a node through a transformed PARENT, which is what makes', () => {
...
it('skew composes between rotation and scale', () => {
...
it('end to end: a resize gesture on a real node holds its opposite corner still', () => {
...
it('Transformer: the frame re-fits itself as the selection changes shape, and does it', () => {
    const node = centredRect({ x: 0, y: 0, width: 100, height: 50 })
...
it('Transformer: the box never describes a set other than the one attached', () => {
...
it('Transformer: the attached set, which the application drives one node at a time', () => {
    resetListenerCensus()
    const t = new Transformer()
...
// findAncestor(s)() ---
it('Node identity: id, name (a space-separated tag list), nodeName/nodeType, and', () => {
...
it('getAttr/setAttr/attrs: string-keyed access to a node\'s typed fields', () => {
...
it('groups: a container that places itself, sized by what it holds', () => {
...
it('an empty group is nowhere, not a point at its own origin', () => {
...
it('the group\'s own transform applies to its contents\' extent', () => {
...
it('a hidden group takes its contents out of the measurement, and out of the scene', () => {
...
it('a layer is NOT a group, which is the entire point of it being its own class', () => {
...
it('a layer is measured through, so a group holding one sizes itself to the contents', () => {
    const group = new Group()
...
it('the transform belongs to Node, so every node in the graph has one', () => {
...
it('the stacking counter: what an unset zIndex means', () => {
...
it('remove(): out of the tree, and still entirely itself', () => {
...
it('removeChildren(): the same thing for a whole container', () => {
    const parent = new Container('holder')
...
it('destroy(): the subtree is finished with', () => {
    resetListenerCensus()
...
it('destroy() announces itself before anything is detached', () => {
    resetListenerCensus()
    const root = new Container('root')
...
it('moveTo(): re-homing, with and without keeping the node where it looks', () => {
    const root = new Container('root')
...
it('keepWorldTransform survives rotation and skew, not just position', () => {
    const root = new Container('root')
...
it('a Transformer lets go of a node that has been destroyed', () => {
    const root = new Container('root')
...
it('...but not of a node that merely moved, nor of one attached before it had a home', () => {
    const root = new Container('root')
...
it('every property that lands in an object record announces itself', () => {
...
it('...and writing the same value back announces nothing', () => {
