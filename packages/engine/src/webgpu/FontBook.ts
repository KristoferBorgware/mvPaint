// FontBook - runtime ownership of the MSDF atlases. All four styles live in ONE
...
export class FontBook {
   ...
   *
   * Stable for the life of the book, which is what lets setFonts() swap the atlases without
  ...
  /**
   * Load against a layout and sampler somebody else owns - what FontLibrary does for every
   * family it holds.
   *
   ...
   *
   *   await handle.setFonts([...handle.getFonts(), { style: 'italic', url, json }])
   *
   ...
   * linger in a layer nobody overwrote. The images are re-fetched, which an HTTP cache makes
   * cheap, and this is a rare call - not something a frame does.
   *
 ...
 *
 * Shared by load() and setFonts() so a book built at startup and one replaced at runtime cannot
  ...
  try {
    ...
    await Promise.all(
      atlases.map(async (source) => {
        ...
        if (layer < 0) throw new Error(`FontBook: '${source.style}' is not one of ${STYLE_ORDER.join(', ')}.`)
        const response = await fetch(source.url)
        if (!response.ok) throw new Error(`FontBook: ${source.url} responded ${response.status}`)
