...
// These run against the committed polygon atlases, so they double as a cross-check between
// two independent extractions of the same font: msdf-bmfont-xml's numbers baked into the
// atlas JSON above, and opentype.js's baked into the polygon atlas by the offline generator
// (packages/scripts/text/polygon). The parser itself is no longer in the engine at all -
// @mvpaint/ttf tests that half, and the generator's own self-test proves the atlas and a live
// parse agree.
// =====================================================================================
...
// --- the atlas is in font units, and its vertical metrics are coherent ---
{
...
// --- an atlas arrives measured: every glyph in the charset, no parsing, no on-demand step ---
{
...
// --- kerning came out of the font's GPOS table and matches what the MSDF generator found ---
{
...
// --- glyph meshes: rings on demand, triangulated once, cached ---
{
