...
//   npx vitest run packages/engine/src/scene/scene.test.ts
...
it('hierarchy composition: child_world parent_local * child_local (column-vector)', () => {
...
//    |   +- a2
//    +- b
it('traversal orders and search over a known tree', () => {
...
it('picking: hitTestShape tests against the shape\'s own tessellated triangles', () => {
...
it('stacking: made later means in front, and zIndex overrides that for both', () => {
    const scene = new Scene()
...
// smallest one.
it('higher zIndex is in FRONT, end to end', () => {
    const scene = new Scene()
...
it('culling: isShapeOnScreen tests WORLD-space bounds overlap against a view', () => {
...
it('marquee: nodesInBox picks up everything a dragged rectangle meets', () => {
    const scene = new Scene()
...
it('a group in a real scene: what draws, what picks, and what a hidden group takes with it', () => {
    const scene = new Scene()
...
it('a hidden group prunes nested subtrees too, however deep', () => {
    const scene = new Scene()
...
it('layers nest, and nest with groups, without any of them learning about the others', () => {
    const scene = new Scene()
...
it('removing and destroying take a node out of everything derived from the scene', () => {
    const scene = new Scene()
...
it('a moved node draws and picks where its new parent puts it', () => {
    const scene = new Scene()
