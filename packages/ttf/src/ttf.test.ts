...
// the committed polygon atlas carries. The comparison is deliberate - the atlas is generated by
// this code (see packages/scripts/textgen/polygon), so if the two ever disagree, every application
// drawing vector text is drawing something the tests never saw.
//
...
const ttf = (name: string): ArrayBuffer => {
  const bytes = readFileSync(new URL(`../../scripts/textgen/fonts/${name}`, import.meta.url))
