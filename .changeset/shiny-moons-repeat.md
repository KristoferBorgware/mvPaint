...
Fonts are the application's, and the atlas the engine ships is now only a fallback.
...
Each family is one array texture, so the text lane emits one draw per family *change* along the
